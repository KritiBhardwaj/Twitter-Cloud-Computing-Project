...
//Drug and Alcohol
//REPLACE API LINK
$responseDA=file_get_contents('http://115.146.95.134:5984/harvester/_design/attribute/_view/alcohol?group_level=1&reduce=true&sorted=true');
...
<script>
  ...
  var pausecontent = new Array();
    ...
    <?php } ?>
		  ...
		  var drugs = new Array();
    <?php foreach($resultDA as $item) {?>
        drugs['<?php echo $item['key']; ?>']= '<?php echo $item['value']; ?>';
    <?php } ?>
	...
	$allSuburbs="[";
	foreach($resultDA as $item) {
	$allSuburbs=$allSuburbs.'"'.$item['key'].'",';
	}
...
$.getJSON('allsuburbsTagged.geo.json', function (geojson) {
    ...
    Highcharts.mapChart('containerMap', {
        ...
        title: {
            text: 'Drug and Alcohol Tweets Suburb Wise'
        },
			...
			point:{
                events:{
					...
					for (var a in drugs)
					{
						if(a==this.name)
						{
						var intvalue = Math.trunc( drugs[this.name] );
						console.log("drugs%: "+intvalue);
  ...
  document.addEventListener("click", function (e) {
	  ...
	  console.log(e.target.textContent);
					...
					for (var a in drugs)
					{
						if(a==mySuburb)
						{
						var intvalue = Math.trunc( drugs[mySuburb] );
						console.log("drugs%: "+intvalue);
