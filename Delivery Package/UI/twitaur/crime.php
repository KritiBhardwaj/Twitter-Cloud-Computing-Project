...
$responseCrime=file_get_contents('http://115.146.95.134:5984/harvester/_design/attribute/_view/crime?group_level=1&reduce=true&sorted=true');
$responseCrime = json_decode($responseCrime, true);
$resultCrime=$responseCrime['rows'];
...
<script>
  ...
  var pausecontent = new Array();
    ...
    <?php } ?>
			  ...
			  var crime = new Array();
    <?php foreach($resultCrime as $item) {?>
        crime['<?php echo $item['key']; ?>']= '<?php echo $item['value']; ?>';
    <?php } ?>
			...
			point:{
                events:{
					...
					for (var a in crime)
					{
						if(a==this.name)
						{
						var intvalue = Math.trunc( crime[this.name] );
						console.log("crime%: "+intvalue);
  ...
  document.addEventListener("click", function (e) {
	  ...
	  console.log(e.target.textContent);
					...
					for (var a in crime)
					{
						if(a==mySuburb)
						{
						var intvalue = Math.trunc( crime[mySuburb] );
						console.log("crime%: "+intvalue);
